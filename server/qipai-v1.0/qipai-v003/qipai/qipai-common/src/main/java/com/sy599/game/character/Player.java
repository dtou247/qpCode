...
//            LogUtil.msgLog.info("statistics:userId=" + userId + ",tableId=" + playingTableId + ",isRecord=" + isRecord + " ,playType=" + playType + ",freeCards=" + freeCards + ",cards=" + cards + ",rest freeCards=" + this.freeCards + ",rest cards=" + this.cards + ",isWrite=" + isWrite + ",temp1=" + temp1 + ",temp2=" + temp2);
    ...
    public void notifyCreditUpdate(long groupId) {
        ...
        writeCreditMessage(groupId, curCredit);

        if(groupUser != null && groupUser.getGroupId() == groupId){
            groupUser.setCredit(curCredit);
        }
    }
	 ...
	 * @param diamondsNum
	 */
	public synchronized void sendDiamonds(long userId, long acpUserid, long diamondsNum,long type){
		try {
			...
			if(isSongZuanQuanxian()){
				...
				if(!result ){
					...
					writeComMessage(WebSocketMsgType.res_code_permission,2,type,"赠送"+acpUserid+" "+diamondsNum+"钻石成功!");
					LogUtil.msgLog.info("sendDiamonds|senduserid|"+getUserId()+"|acpuserid|"+acpUserid+"|"+diamondsNum);
					try {
						//通知被赠送人钻石变动
						Player player1 = PlayerManager.getInstance().getPlayer(acpUserid);
						if (player1 != null) {
							ServerUtil.notifyPlayerCards(player1.getEnterServer(), acpUserid, type==2?diamondsNum:0, type==1?diamondsNum:0);
						} else {
							RegInfo user = UserDao.getInstance().selectUserByUserId(acpUserid);
							if (user != null) {
								if (user.getIsOnLine() == 1 && user.getEnterServer() > 0) {
									ServerUtil.notifyPlayerCards(user.getEnterServer(), user.getUserId(), type==2?diamondsNum:0, type==1?diamondsNum:0);
								}
							}
						}
					}catch (Exception e){
						writeErrMsg("通知失败，接收人"+acpUserid);
					}
				}
			}else{
				writeErrMsg("权限不足");
...
//		}
	...
	public String toXipaiDatatoInfoStr() {
		...
		sb.append(getXiPaiReady()).append(",");
		return sb.toString();
	}
	/**
	 * 解析洗牌数据
	...
	public void initXipaiDataPlayInfo(String data) {
		if (!StringUtils.isBlank(data)) {
			...
			this.xiPaiReady = StringUtil.getIntValue(values, end--);
			return;
		}
	}
	...
	public int getXiPaiReady() {
		return xiPaiReady;
	}

	public void setXiPaiReady(int xiPaiReady) {
		this.xiPaiReady = xiPaiReady;
	}
}
