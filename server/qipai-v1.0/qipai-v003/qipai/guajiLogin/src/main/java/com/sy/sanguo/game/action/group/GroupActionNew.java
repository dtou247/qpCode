...
//        if(!jsonObject.containsKey("rbr")){
//            map.put("redbagrain", "0");
//        }else{
//           String rbr = (String) jsonObject.get("rbr");
//          if( rbr.equals("0")||rbr.equals("1")){
//              map.put("redbagrain", rbr);
//          }else{
//              map.put("redbagrain", "0");
//          }
//        }
    ...
    public void userListNextLevel() {
        try {
            ...
            if (self == null) {
                ...
                return;
            } else if (GroupConstants.isForbidden(self)) {
...
//                return;
    ...
    public void creditLogList() {
        ...
        try {
            ...
            int dataCount = groupDaoNew.countCreditLogList(groupId, targetId, selectType, startDate, endDate, upOrDown,isLookXipai);
            long sumCredit = 0;
            List<Map<String, Object>> dataList = Collections.emptyList();
            if (dataCount > 0) {
                dataList = groupDaoNew.creditLogList(groupId, targetId, selectType, startDate, endDate, upOrDown, pageNo, pageSize,isLookXipai);
                sumCredit = groupDaoNew.sumCreditLogList(groupId, targetId, selectType, startDate, endDate, upOrDown,isLookXipai);
            }
            JSONObject json = new JSONObject();
            json.put("pageNo", pageNo);
            json.put("pageSize", pageSize);
    ...
    public void updateCommissionConfig() {
        ...
        try {
            ...
            //记录玩家已经修改过此值
            updateGroupUserExt(target, GroupConstants.extKey_commissionConfig, "1");
        ...
        try {
            ...
            if (optType == 1) {
                ...
                switch (optType) {
                    ...
                    case 15:
                        // 允许普通成员退群
                        key = GroupConstants.groupExtKey_allowQuit;
                        break;
                }
                if (null == key) {
                    OutputUtil.output(4, LangMsg.getMsg(LangMsg.code_3), getRequest(), getResponse(), false);
...
//     * 查询 亲友圈下级当天比赛分数据
//     */
//    public  void loadLogGroupCommissionData(){
//        Map<String, String> params = null;
//        try {
//            params = UrlParamUtil.getParameters(getRequest());
//            LOGGER.debug("loadGroupCreditData|params:{}", params);
//            if (!checkSign(params)) {
//                OutputUtil.output(-1, LangMsg.getMsg(LangMsg.code_1), getRequest(), getResponse(), false);
//                return;
//            }
//            long userId = NumberUtils.toLong(params.get("userId"), 0);//120098;//
//            RegInfo user = checkSessCodeNew(userId, params.get("sessCode"));
//            if (user == null) {
//                OutputUtil.output(-2, LangMsg.getMsg(LangMsg.code_2), getRequest(), getResponse(), false);
//                return;
//            }
//            long groupId =NumberUtils.toLong(params.get("groupId"), 0);//3071;//
//            String startDate = params.get("date"); // 日期格式：2019-08-01 00:00:01
//            String startDateyyyyMMdd = TimeUtil.getSimpleDay(TimeUtil.parseTimeInDate(startDate));
//            Map<String, Object> map = new HashMap<>();
//            map.put("groupId", groupId);
//            map.put("userId", userId);
//            map.put("date", startDate);
//            Object  l = groupDaoNew.loadLogGroupCommissionData(map);
//            Object returnData =JSONObject.toJSON(l);
//            System.out.println("returnData==>");
//            System.err.println(returnData);
//            OutputUtil.output(0, returnData , getRequest(), getResponse(), false);
//        }catch (Exception e){
//            e.printStackTrace();
//            OutputUtil.output(1, LangMsg.getMsg(LangMsg.code_4), getRequest(), getResponse(), false);
//            LOGGER.error("loadGroupCreditData|error|" + JSON.toJSONString(params), e.getMessage(), e);
//        }
//    }
...
//           }
    ...
    public void quitGroup(){
        try {
            Map<String, String> params = UrlParamUtil.getParameters(getRequest());
            LOGGER.info("quitGroup|params:{}", params);
            if (!checkSign(params)) {
                OutputUtil.output(-1, LangMsg.getMsg(LangMsg.code_1), getRequest(), getResponse(), false);
                return;
            }
            ...
            if(group.getExtMsg().contains(GroupConstants.groupExtKey_allowQuit)){
                JSONObject json = StringUtils.isBlank(group.getExtMsg()) ? new JSONObject() : JSONObject.parseObject(group.getExtMsg());
                String op = json.getString(GroupConstants.groupExtKey_allowQuit);
                if(!"1".equals(op)){
                    OutputUtil.output(2,  "普通成员退出未开启!", getRequest(), getResponse(), false);
                    return ;
                }
            }else{
                OutputUtil.output(2,  "普通成员退出未开启!", getRequest(), getResponse(), false);
            ...
            if(groupUser.getUserRole()!=90000  ){
                OutputUtil.output(2,  "您不是普通成员!", getRequest(), getResponse(), false);
                return;
            }
            if(groupUser.getCredit()!=0){
                OutputUtil.output(2,  "分值不为0,不能退出!", getRequest(), getResponse(), false);
                return;
            }
            int ret = groupDao.deleteGroupUserByKeyId(groupUser.getKeyId());
            if(ret==1){
                OutputUtil.output(0, "退出成功!", getRequest(), getResponse(), false);
                LOGGER.info("quitGroup|success", params);
            }
        }catch (Exception e){
            OutputUtil.output(2, "系统错误：请联系管理员", getRequest(), getResponse(), false);
            LogUtil.e("quitGroup|error|", e);
        }
    }
}
