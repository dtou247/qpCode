...
//                }
    ...
    public void fireUser() {
        try {
            ...
            if (GroupConstants.isHuiZhang(groupUser.getUserRole())) {
                ...
                if (sumCredit != 0) {
                    response(5, "操作失败:该成员或成员下级的比赛分不为0");
                    return;
                }
    ...
    public void changeUserCurrency() {
        try {
            ...
            long retFreeCard = 0;
            if (cards != 0) {
                retCard = UserDao.getInstance().changeUserCards(userId, cards, changeType);
            }
            if (freeCards != 0) {
                retFreeCard = UserDao.getInstance().changeUserFreeCards(userId, freeCards, changeType);
            }
            MessageBuilder msg = MessageBuilder.newInstance();
            msg.builderCodeMessage(0, LangMsg.getMsg(LangMsg.code_0));
            msg.builder("retCard", retCard);
            msg.builder("retFreeCard", retFreeCard);
            response(msg);
            LOGGER.info("BjdAction|changeUserCurrency|succ|" + userId + "|" + retCard + "|" + params);
        } catch (Exception e) {
            response(2, LangMsg.getMsg(LangMsg.code_4));
    ...
    public void selfRegister() {
        try {
            ...
            msg.builder(result);
            response(msg);
            LOGGER.info("BjdAction|selfRegister|succ|" + JSONObject.toJSONString(result) + "|" + params);
        } catch (Exception e) {
            response(2, LangMsg.getMsg(LangMsg.code_4));
