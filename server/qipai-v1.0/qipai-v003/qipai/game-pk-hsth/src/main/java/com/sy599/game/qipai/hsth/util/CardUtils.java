...
//        System.out.println(  checkTongHuBuChai(Arrays.asList(105,305,205),deskcards,4));
    }
...
//        ls.add(minboom);
    ...
    public static boolean checkTongHuaBuChai(List<Integer> cards,List<Integer> handcards,int op_cardsnum) {

            HashMap<Integer,Integer> val_num = new HashMap<>();
            for (int c:cards){
                if(val_num.containsKey(c)){
                    val_num.put(c,val_num.get(c)+1);
                }else{
                    val_num.put(c,1);
                }
            }
            ...
            int min = op_cardsnum>=10?5:4;
            //获取出的手牌里面是否有多个同花；
            List<List<Integer>> outTongHaveHua=getHandTongHuaBoom(cards,min);
            if(null!=outTongHaveHua&& outTongHaveHua.size()>=2){
                return  true;//双同花不可做8炸出
            }
            if(null!=outTongHaveHua && outTongHaveHua.size()==1){
               int length = outTongHaveHua.get(0).size();
            ...
            List<List<Integer>> allTongHua = getHandTongHuaBoom(handcards,min);
            for (int c:cards ) {
                for(List<Integer> ls :allTongHua){
                    if(ls.contains(c)){
                        //同花中包含出的单张
                        if(val_num.get(c)<ls.size()){
                            return  true;
                        }
                    }
                }
