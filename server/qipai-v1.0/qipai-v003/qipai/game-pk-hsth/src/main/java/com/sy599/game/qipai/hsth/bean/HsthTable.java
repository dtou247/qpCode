...
public class HsthTable extends BaseTable {
	...
	private int op_buYao8;
	private int op_tonghuaBuKeChai;
...
//			resetplayInfo();
	...
	public JsonWrapper buildExtend0(JsonWrapper wrapper) {
		...
		wrapper.putInt(30, op_buYao8);
        wrapper.putInt(31, op_tonghuaBuKeChai);
		return wrapper;
	}
    @Override
    public void initExtend0(JsonWrapper wrapper) {
        ...
        op_buYao8=wrapper.getInt(30, 0);
        op_tonghuaBuKeChai=wrapper.getInt(31, 0);
	}
	...
	public void fapai() {
		...
		synchronized (this) {
			...
			timeNum = 0;
			List<List<Integer>> fplist = CardTool.fapai(4,zp,op_cardsnum,op_buYao8);
...
//      seatMap.get(getNowDisCardSeat()).getHandPais().isEmpty()
	...
	private int chupai(HsthPlayer player, int action, List<Integer> cards) {
		...
		if(this.op_tonghuaBuKeChai==1){
		 boolean chai =	CardUtils.checkTongHuaBuChai(cards,player.getHandPais(),op_cardsnum);
		 if(chai){
			 player.writeErrMsg("出牌不符合规则。");
		 	return -1;
		 }
...
//		ComRes.Builder builder = SendMsgUtil.buildComRes(WebSocketMsgType.DUZHAN_TEAM, jarr.toJSONString());
	...
	public boolean createTable(Player player, int play, int bureauCount, List<Integer> params, boolean saveDb)
		...
		this.op_buYao8 = StringUtil.getIntValue(params, 16, 0);
		this.op_tonghuaBuKeChai = StringUtil.getIntValue(params, 17, 0);
		setLastActionTime(TimeUtil.currentTimeMillis());
		return true;
	}
