package com.sy599.game.qipai.hsth.tool;

import com.sy599.game.GameServerConfig;
import com.sy599.game.qipai.hsth.constant.HsthConstants;
import com.sy599.game.qipai.hsth.util.CardType;
...
public final class CardTool {
	 ...
	 * @return
	 */
	public static List<List<Integer>> fapai(int playerCount, List<List<Integer>> zps, int op_cardsnum,int buyao8) {
		List<List<Integer>> list = new ArrayList<>();
		List<Integer> copy = null;
		if(op_cardsnum==8){
			copy = new ArrayList<>(HsthConstants.cardList_8);
		}else if(op_cardsnum==9){
			copy = new ArrayList<>(HsthConstants.cardList_9);
		}else if(op_cardsnum==10){
			copy = new ArrayList<>(HsthConstants.cardList_10);
		}else if(op_cardsnum==12){
			copy = new ArrayList<>(HsthConstants.cardList_12);
		}else if(op_cardsnum==13){
			copy = new ArrayList<>(HsthConstants.cardList_13);
		}else if(op_cardsnum==14){
			copy = new ArrayList<>(HsthConstants.cardList_14);
		}
		if(buyao8 ==1){
			//新增发牌去掉8
			List<Integer> val8 = Arrays.asList(108,208,308,408);
			copy.removeAll(val8);
		}
